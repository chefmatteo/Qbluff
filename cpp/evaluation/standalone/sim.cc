...
Compile:
 ...
 cd cpp && clang++ -std=c++17 -O3 -I./include -o evaluation/standalone/sim evaluation/standalone/sim.cc -L. -lpheval
...
const std::unordered_map<int, char> cardRankMap = {
    {0, '2'},
    {1, '3'},
    {2, '4'},
    {3, '5'},
...
int main(int argc, char **argv)
{
  ...
  for (auto c0 = 12; c0 >= 0; --c0)
  {
    for (auto c1 = 12; c1 >= 0; --c1)
    {
      ...
      if (c0 > c1)
      {
        std::cout << cardRankMap.at(c0) << cardRankMap.at(c1) << "s ";
      }
      else if (c1 > c0)
      {
        std::cout << cardRankMap.at(c0) << cardRankMap.at(c1) << "o ";
      }
      else
      {
        std::cout << cardRankMap.at(c0) << cardRankMap.at(c1) << "  ";
      }
      std::cout << "\033[0m";
